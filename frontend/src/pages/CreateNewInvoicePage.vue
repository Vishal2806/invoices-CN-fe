...
const items = ref([
  { id: 1, product: 'Note Book (200 pages)', rate: 120, quantity: 20, discount: 200, total: 2400 },
...
const nameRules = [
  ...
  (v: string) => /^[a-zA-Z\s]+$/.test(v) || 'Name cannot contain special characters or numbers'
]
...
const addItem = () => {
  ...
  items.value.push({ id: newId, product: '', rate: 0, quantity: 0, discount: 0, total: 0 })
}
...
const nextStep = () => {
  if (currentStep.value < totalSteps) {
    if (currentStep.value === 1 && step1Valid.value) {
      currentStep.value++
    } else if (currentStep.value === 2 && items.value.every(item => item.product && item.quantity > 0)) {
      currentStep.value++
    }
  }
}
  ...
  <div class="invoice-layout">
    ...
    <div class="main-content">
      ...
      <div class="stepper-container">
        ...
        <!-- Step Content -->
        <div class="step-content pa-6" style="background-color: #ffffff; color: #333;">
          <!-- Step 1: Invoice Details -->
          <div v-if="currentStep === 1" class="step-1">
            <v-form v-model="step1Valid">
              <!-- Company Details Section -->
              <div class="section-title text-primary font-weight-bold mb-4">Company Details</div>
              ...
              <div class="section-title text-primary font-weight-bold mb-4">Invoice To Details</div>
              
              <div class="mb-4">
                <div class="text-subtitle-2 mb-1">Payment Mode</div>
                <v-select
                  v-model="invoiceDetails.paymentMode"
                  :items="paymentModes"
            ...
            <div class="d-flex justify-end mt-6">
              <v-btn color="primary" rounded @click="nextStep" :disabled="!step1Valid">
                Next
              </v-btn>
            </div>
          </div>
          ...
          <div v-if="currentStep === 2" class="step-2">
            <div class="section-title text-primary font-weight-bold mb-4">Transaction Details</div>
            ...
            <div class="d-flex justify-end gap-4 mt-6">
              <v-btn color="secondary" variant="outlined" rounded @click="prevStep">
                Back
              </v-btn>
              <v-btn color="primary" rounded @click="nextStep" :disabled="!step2Valid">
                Next
              </v-btn>
            </div>
          </div>
            ...
            <div class="invoice-preview">
              <div class="d-flex justify-space-between mb-4">
                <div>
                  ...
                  <h1 class="text-h4 text-green font-weight-bold mt-4">Invoice</h1>
                </div>
                <div class="text-right">
                  ...
                  <div class="mt-4">Date: {{ new Date().toLocaleDateString('en-GB').split('/').join('-') }}</div>
                </div>
              </div>
              ...
              <div class="invoice-to mt-8 mb-6">
                ...
                <div>Address: {{ invoiceDetails.recipientAddress }}, {{ invoiceDetails.recipientCity }}, {{ invoiceDetails.recipientState }} {{ invoiceDetails.recipientPincode }}</div>
              </div>
...
.rate-col, .quantity-col, .discount-col, .total-col {
  flex: 1;
  padding: 0 4px;
  display: flex;
  align-items: center;
